...
/* A string buffer */
typedef struct { char *data; size_t len; } sbuf;

static void sbuf_init(sbuf *b){ b->data=NULL; b->len=0; }
static void sbuf_free(sbuf *b){ free(b->data); b->data=NULL; b->len=0; }
...
static char *sanitize_utf8(const unsigned char *src, size_t n) {
    ...
    while (p < end) {
        ...
        if (c < 0x80) {  // ASCII
            if (c == '\n' || c == '\r')        *w++ = (char)c;
            else if (c < 0x20 || c == 0x7f)    *w++ = ' ';
            else                                *w++ = (char)c;
            p++; continue;
        }

        size_t need = 0; unsigned char c1=0, c2=0, c3=0;
        if ((c & 0xE0) == 0xC0) {                               // 2-byte
            if (end - p >= 2) { c1 = p[1];
                if (c >= 0xC2 && c <= 0xDF && (c1 & 0xC0) == 0x80) need = 2;
            }
        } else if ((c & 0xF0) == 0xE0) {                        // 3-byte
            if (end - p >= 3) { c1 = p[1]; c2 = p[2];
            ...
            if (end - p >= 4) { c1 = p[1]; c2 = p[2]; c3 = p[3];
                if (((c == 0xF0 && c1 >= 0x90 && c1 <= 0xBF) ||
                     (c >= 0xF1 && c <= 0xF3 && (c1 & 0xC0) == 0x80) ||
                     (c == 0xF4 && c1 >= 0x80 && c1 <= 0x8F)) &&
                    ((c2 & 0xC0) == 0x80) && ((c3 & 0xC0) == 0x80)) need = 4;
            }
        }
        if (need) { for (size_t i=0;i<need;i++) *w++ = (char)p[i]; p += need; }
        else { *w++='?'; p++; }
    }
    *w = '\0';
    return out;
...
static int looks_text(const unsigned char *buf, size_t n) {
    ...
    for (size_t i=0;i<n;i++){
        if (buf[i] == 0) return 0;
        if (!(isprint(buf[i]) || isspace(buf[i]))) non++;
    }
...
static int read_head(const char *path, char **out, size_t max_bytes) {
    ...
    fclose(f);
    if (n == 0 || !looks_text(buf, n)) { free(buf); return -2; }
...
static void walk_dir(const char *root, size_t max_bytes, cJSON *files_arr, int *count) {
    ...
    struct dirent *ent;
    while ((ent = readdir(d))) {
        if (is_hidden(ent->d_name)) continue;
        char path[4096];
        ...
        if (S_ISDIR(st.st_mode)) {
            ...
            if (*count >= DIRSUM_MAX_FILES) break;
...
static char* build_request_json(cJSON *files, const char *model) {
    /* Payload needs to look like this:
       { "model": "...", "input": [ {role:"system"...},{role:"user",content:"<json>"} ], "stream": true } */
...
static void sse_state_init(sse_state *st) { sbuf_init(&st->linebuf); sbuf_init(&st->blockbuf); }
static void sse_state_free(sse_state *st) { sbuf_free(&st->linebuf); sbuf_free(&st->blockbuf); }
...
static void sse_process_block(const char* blk, size_t n) {
    ...
    const char *data_start = NULL; size_t data_len = 0;

    while (p < end) {
        const char *nl = memchr(p, '\n', (size_t)(end - p));
        size_t len = nl ? (size_t)(nl - p) : (size_t)(end - p);
        while (len && p[len-1] == '\r') len--;  // trim CR
    ...
    if (!data_start || data_len == 0) return;
...
static size_t sse_write_cb(char *ptr, size_t size, size_t nmemb, void *userdata) {
    ...
    for (size_t i = 0; i < st->linebuf.len; ++i) {
        if (buf[i] == '\n') {
            ...
            while (trimmed && buf[start + trimmed - 1] == '\r') trimmed--;
            ...
            if (trimmed == 0) {
                if (st->blockbuf.len > 0) {
                    ...
                    st->blockbuf.len = 0;
                    if (st->blockbuf.data) st->blockbuf.data[0] = '\0';
                }
            } else {
...
static int http_post_openai_stream(const char *json_payload) {
    ...
    if(!api_key){ fprintf(stderr, "dirsum: set OPENAI_API_KEY\n"); return -1; }

    CURL *curl = curl_easy_init();
    if(!curl){ fprintf(stderr, "dirsum: curl init failed\n"); return -1; }
    ...
    sse_state st; sse_state_init(&st);
...
int do_dirsum(int argc, char **argv) {
    ...
    for (int i=1;i<argc;i++){
        ...
        else if (!strcmp(argv[i], "-n") && i+1<argc) { max_files = atoi(argv[++i]); }
        else if (!strcmp(argv[i], "-b") && i+1<argc) { head_bytes = (size_t)atol(argv[++i]); }
    ...
    if (count == 0) {
        if (!debug) fprintf(stderr, "dirsum: no text-like files found under '%s'\n", root);
        cJSON_Delete(files);
        return 0;
    }
